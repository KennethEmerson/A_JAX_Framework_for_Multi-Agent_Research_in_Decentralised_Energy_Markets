...
-------------------------------------------------------------------------------------------
File contains the implementation of the selfplay PPO agent class
...
from agents.agent_config import PpoAgentConfig
...
class SelfplayPpoAgent(Agent):
    """ Agent Class implementing a selfplay Proximal policy optimization agent
    """
    def __init__(
              self,
        ...
        """ instantiates the selfplay Agent object
        ...
        Args:
            ...
            num_agents (int): the number of agents/actors sharing the neural net
        """
        ...
        # NOTE this is different from a regular PPO agent
        self.minibatch_size = self.num_agents * self.train_config.NUM_STEPS // self.agent_config.NUM_MINIBATCHES
    ...
    def _calculate_gae(self,traj_batch:_Transition, last_val:float)-> Tuple[jax.Array,jax.Array]:
        ...
        _, advantages = jax.lax.scan(
            ...
            traj_batch,
            reverse=True,
            unroll=16, # is this the lookforward parameter from PPO?
        )
    ...
    def _calc_advantages_and_targets(
              ...
              traj_batch=_Transition) -> Tuple[jax.Array,Any]:
    ...
    def create_init_train_state(self,observation_space_size:int,rng:jax.Array) -> TrainState:
        """ initializes the flax.training.train_state of the selfplay agent
    ...
    def select_action(
        ...
        """returns the selected action by the selfplay agent based on its trainstate and the given observations
    ...
    @functools.partial(jax.jit, static_argnums=(0,))
    def update_networks(self,update_state)-> Tuple[_UpdateState,_Loss]:
        ...
        # NOTE this is different from regular PPO agent
        minibatch_size = self.num_agents * self.train_config.NUM_STEPS // self.agent_config.NUM_MINIBATCHES
